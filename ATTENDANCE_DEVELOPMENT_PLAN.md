...
避免服务重启或 Cron 暂停造成漏记。学生在 `11:00:00` 后尝试打卡时，也先
幂等确保当天缺勤记录存在，再返回“已过打卡时间”。
...
验收：重复运行任务不会重复写入，`11:01` 后无记录学生变为缺勤。
...
11. 管理员可以维护假期和办公网络。
12. HR 可以通过“共享文档”页面访问配置的腾讯文档；禁止嵌入时可在新标签
    页打开。
13. 核心时间边界、权限、IP、设备和并发测试通过。
