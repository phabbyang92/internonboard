    ...
    <section className="border border-[#d3ddda] bg-white">
      ...
      {isEditing ? (
        <form className="space-y-4 p-5" onSubmit={submit}>
          ...
          <label className="block text-xs font-semibold text-[#52615d]">入职开始日期
            <input required disabled={student.onboardingStatus === "onboarded"} min={getChinaTodayInput()} type="date" value={startAt} onChange={(e) => setStartAt(e.target.value)} className="mt-2 h-10 w-full border border-[#bdcac6] px-3 text-sm disabled:bg-[#f1f4f3]" />
          </label>
        ...
        <dl className="grid gap-4 p-5 text-sm">
          ...
          <div><dt className="text-xs text-[#75817d]">入职开始日期</dt><dd className="mt-1 font-medium">{formatDateOnly(student.onboardingStartAt)}</dd></div>
