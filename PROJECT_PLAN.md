...
找到匹配记录且 HR 已安排工作地点和入职开始日期 -> 允许进入登记表
...
- 匹配到学生记录，且 HR 已安排工作地点和入职开始日期：进入登记表。
...
- `candidate`：HR 已确认学生接受 offer，并录入系统，但尚未完成工作地点和入职开始日期安排。
...
```text
如果 onboardingStartAt 所代表的日期 <= 当前日期
...
- `onboardingStartAt`：HR 在学生填表前安排的入职开始日期，数据库中按中国时区当天零点保存。
...
- `GET /api/student/form`：获取登记表内容、HR 已安排地点、入职开始日期。
